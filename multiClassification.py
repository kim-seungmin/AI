...
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=model_LC, labels=Y))
...
with tf.Session() as sess:
    ...
    test2 = sess.run(model, feed_dict={X: [[1, 11, 7, 9], [1, 3, 4, 3], [1, 1, 0, 1]]})
    print(test2, sess.run(tf.argmax(test2, 1)))